...
while cap.isOpened():
    ...
    # 2) Ball detection & dribble counting
    ...
    # 3) Pose detection
    #    — get the first result and raw tensor
    pose_results = pose_model(frame, verbose=False, conf=0.5)
    res = pose_results[0]
    # raw torch.Tensor of shape [N,17,3]
    kps_tensor = res.keypoints.data

    # if no keypoints, skip
    if kps_tensor is None or kps_tensor.numel() == 0:
        ...
        rounded_results = np.round(kps_np, 1)  # shape e.g. (1,17,3)
